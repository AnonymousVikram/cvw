#include "../common/util.h" // supports verify
#include <stdint.h>         // for fixed width ints
#include <stdio.h>          // supports printf

extern void fir(int x[], int c[], int y[], int n, int m);

//* Cycles without optimization: 789
//* Cycles with optimization:  788

// Add two Q1.31 fixed point numbers
int add_q31(int a, int b) { return a + b; }

// Multiply two Q1.31 fixed point numbers
int mul_q31(int a, int b) {
  int64_t product = (int64_t)a * (int64_t)b;
  // printf("mul_q31: a = %x, b = %x, product = %lx\n", a, b, product);

  int32_t result = (int32_t)((product << 1) >> 32);
  return result;
}
...
void fir(int x[], int c[], int y[], int n, int m) {
  // There will be n - m + 1 output samples.
  int num_outputs = n - m + 1;
  for (int j = 0; j < num_outputs; ++j) {
    int sum = 0;
    for (int i = 0; i < m; ++i) {
      // Multiply the input sample by the corresponding coefficient and
      // accumulate.
      sum = add_q31(sum, mul_q31(x[j - i + m - 1], c[i]));
    }
    y[j] = sum;
  }
}
...
int main(void) {
  ...
  int lowpass[4] = {0x20000001, 0x20000002, 0x20000003,
                    0x20000004}; // 1/4 in Q1.31 format
  int y[17];
  int expected[17] = {
      // in Q1.31 format
      0x4fad3f2f, 0x627c6236, 0x4fad3f32, 0x1e6f0e17, 0xe190f1eb, 0xb052c0ce,
      0x9d839dc6, 0xb052c0cb, 0xe190f1e6, 0x1e6f0e12, 0x4fad3f2f, 0x627c6236,
      0x4fad3f32, 0x1e6f0e17, 0xe190f1eb, 0xb052c0ce, 0x9d839dc6};
